...
For entity extraction, we used AIDA \citep{hoffart2011robust}, a framework and online tool for entity detection and disambiguation. %AIDA uses the Stanford NER tagger \cite{finkel2005incorporatingfull} to locate entity candidates, and attempts to link them to existing entities within the YAGO2 knowledge base.
...
\begin{itemize}
	...
	\citep{cheng2011automatedfull} devise a generic propositionalization framework  using linked data via relation-based queries.% and offer an insight into using taxonomy-based features by creating features that list the taxonomic class relationships of a given entity.
	...
	\item Data mining approaches such as FeGeLOD \citep{paulheim2012unsupervisedfull} aim to directly utilize linked data in order to automatically enrich existing data. % yielding additional information on existing entities within the data. 

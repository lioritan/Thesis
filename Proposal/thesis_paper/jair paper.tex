...
%As in the previously described algorithms, lmdt (Utgoff & Brodley, 1991) performs feature construction
%in the course of building a decision-tree classifier. At each created tree node, the algorithm constructs a
%hyperplane feature by training a thermal linear machine. The construction procedure is aimed at generating
%concise hyperplanes that are based on relevant data attributes. When lmdt detects that a linear machine is
%near its final set of boundaries, it eliminates the variable which least contributes to discriminating between
%elements of the two classes in the current set of instances. Afterwards, it continues training the linear
%machine. Finally, the most accurate linear machine with the minimum number of variables is chosen.
%The lfc algorithm performs feature construction throughout the course of building a decision tree classifier. New features are constructed at each created tree node by performing a branch and bound search in
%feature space. The search is performed by iteratively combining the feature having the highest InfoGain
%value with an original basic feature that meets a certain filter criterion. The constructed features may be
%used in the generated tree classifier, which is returned as the final output of the algorithm.
...
%We focused our work on the text categorization problem. The problem of using texts for machine learning tasks
% is of great interest, as the applications are plentiful and many possible approaches exist. A standard approach to this problem is 
 %now we can talk about it+talk about linked data?
 
 %Constructing features from linked data has been discussed in both theory \cite{tresp2008towards} and practice \cite{kappara2011liddm,cheng2011automatedfull,paulheim2012unsupervisedfull}, 
 %achieving various positive results. This topic has not, however been discussed at length with the problem of text categorization in mind.
 %Unlike most entity-based induction tasks, where connecting external knowledge to the problem is well understood, tasks dealing in text directly must first extract semantically meaningful entities for it. 
 %There are several methods of doing so, from Named Entity Recognition \cite<see>{nadeau2007survey} methods, through enriching approaches such as ESA \cite{gabrilovich2009wikipediafull}, to Entity Linking \cite{rao2013entity}.
 %All of these approaches seek to match semantic entities to a given text, 

%TODO:talk about entity linking? talk about linked data?

%Several special-purpose algorithms were designed for specific problem domains (Hirsh & Japkowicz, 1994;
%Sutton & Matheus, 1991). These algorithms construct special-purpose features using domain-specific background knowledge. Such an example is the bootstrapping algorithm (Hirsh & Japkowicz, 1994), designed
%especially for the domain of molecular biology. The algorithm represents features as nucleotides sequences
%whose legal syntax structure is determined by existing background knowledge. The algorithm starts with an
%initial set of feature sequences, produced by human experts, and uses a special set of operators to alter them
%into new sequence features. Such special-purpose algorithms may be effectively tailored for a given domain,
%but may be hard to generalize to other domains and problems.

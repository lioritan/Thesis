...
%, they are roughly comparable, and in some cases perform noticeably better. One particular such case is that of K-NN for the OHSUMED dataset, which was previously the only case where no improvement in accuracy was achieved. Using an SVM classifier to generate features rather than tree-based features, we see an improvement in accuracy for this case. This may be due to the nature of RBF-based classifiers, which look at distance between examples, similarly to K-NN.
...
We note that there are 46 available relations, and thus the number of features we can expect to look at is at most 46 (if a relation does not apply to a sub=problem, it is not counted in this average). %lvl2: it almost always makes a feature 42/42, and lvl2 features we have
...
Lastly, we look at the local IG of the newly created features. In figure \ref{fig:local_ig_depth}, we see the mean information gain of generated features per depth, compared to the best information gained achieved by a non-recursive feature for that depth. We see that our recursive features tend to have a much higher information gain, especially as tree depth decreases. We see a decrease in both cases as depth increases, due to the fact that it is more difficult to find distinguishing features as depth increases.

\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{local_ig_depth}
	\caption{Mean information gain of newly created FEAGURE feature compared to the best information gain for original learning problem.}
	\label{fig:local_ig_depth}
\end{figure}

\subsection{Qualitative Analysis}

In order to better understand the contribution of these features, let us look at several of them to try and better understand them. Figure\ref{fig:feature_eastcoast_rel-ownsproperty} shows a simple constructed feature on entities answering the ``owns" relation, which combines people and organizations. We see that the use of the ``located in" relation, alongside a relatively broad area such as the east coast of the USA allows us to capture a powerful concept with ease, thanks to the expressiveness of the knowledge base. This feature, designed to separate texts focusing on Michigan businesses from those discussing locations in Arizona, allows us to almost perfectly capture the concept at hand.

\begin{figure}
	\centering
	\includegraphics[width=0.6\linewidth]{feature_eastcoast_rel-ownsproperty}
	\caption{Recursive feature constructed by FEAGURE for entities in the ``owns" relation.}
	\label{fig:feature_eastcoast_rel-ownsproperty}
\end{figure}

%\begin{figure}
%	\centering
%	\includegraphics[width=0.8\linewidth]{feature_governor_rel-politician}
%	\caption{A}
%	\label{fig:feature_governor_rel-politician}
%\end{figure}

The feature displayed in Figure \ref{fig:feature_eastcoast_rel-ownsproperty} is a simple one, and thus can be easily found by a feature generation algorithm that makes use of the knowledge base. The features in Figures \ref{fig:feature_celeb_rel-livesin} and \ref{fig:feature_celeb2_rel-actedin}, on the other hand, are more complex. The first of these is a feature that utilizes type relations (also known as ``is-a" relations or hypernyms) to group together several types of celebrity figures, namely actors, directors and models.
The second of these features is perhaps more interesting, as it only operates on entities in the ``acted in" relation, meaning these are all former actors. These former actors are then separated into subgroups: musicians, producers and, strangely enough, vegetarians. While this last separation might seem strange, it is possible that this feature was somehow useful in distinguishing the two groups.

\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{feature_celeb_rel-livesin}
	\caption{Recursive feature constructed by FEAGURE for entities in the ``lives in" relation.}
	\label{fig:feature_celeb_rel-livesin}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{feature_celeb2_rel-actedin}
	\caption{Recursive feature constructed by FEAGURE for entities in the ``acted in" relation.}
	\label{fig:feature_celeb2_rel-actedin}
\end{figure}

Figure \ref{fig:feature_bad_rel-marriedto} shows a poorly fitted feature constructed by FEAGURE. We see that many of the sub-categories used are narrow and only apply to specific individuals. There are several features such as this, and these can be narrowed through the use of filtering techniques.

\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{feature_bad_rel-marriedto}
	\caption{Recursive feature constructed by FEAGURE for entities in the ``married to" relation.}
	\label{fig:feature_bad_rel-marriedto}
\end{figure}

No analysis of FEAGURE would be complete without an example of a recursive feature constructed for a recursive induction problem. Figure \ref{fig:feature_compound_clf_rel-reversebornin} shows a feature for entities in the relation ``born in", using the location as key, rather than the person. In order to solve this recursive problem on locations, FEAGURE generated a new problem using the ``died in" relation, creating a problem on individuals who died in those locations. This new problem looks at individuals who died in the city of Los Angeles, former army officers, and well-known American vegetarian Johnny Appleseed. The result of this classifier can be either true, false or inapplicable (for example, for people who are alive).

\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{feature_compound_clf_rel-reversebornin}
	\caption{Recursive feature constructed by FEAGURE for entities in the ``born in" relation. The rightmost arrow shows the 2-level classifier for these entities.}
	\label{fig:feature_compound_clf_rel-reversebornin}
\end{figure}

Finally, Figure \ref{fig:feature_mixed_rel-reverse_located_in} shows a mixed feature, combining a single and two-level feature. This feature on locations first checks whether these locations are within the state of Texas. If they are not, it checks whether they were a site of a conflict or battle. We can easily see that this feature captures a powerful concept, and can be highly useful for classification.

\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{feature_mixed_rel-reverse_located_in}
	\caption{Recursive feature constructed by FEAGURE for entities in the ``located in" relation. The rightmost arrow shows the 2-level classifier for these entities.}
	\label{fig:feature_mixed_rel-reverse_located_in}
\end{figure}

To conclude, we see that in general, good features created by FEAGURE are those that are sufficiently broad in scope to group together a sizeable subset of similar entities. To this end, hypernyms are very useful, as they, by definition, group together entities in a taxonomy. 

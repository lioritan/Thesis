...
%\cite{cheng2011automatedfull,paulheim2012unsupervisedfull}. \citeA{cheng2011automatedfull} devise a theoretical framework for constructing features from linked data. By specifying entity types relevant to the problem at hand, they restrict the space of possible features to a more manageable size, and allow for the creation of a reasonably small amount of features. They also note that this approach tends to lead to highly sparse feature vectors.


%In the past, before the overwhelming popularity of inductive learning techniques, another school of learning methods, exemplified by works such as Explanation-Based learning , was explored as a way to allow automated learning of concepts. These methods relied on deduction of general concepts through the use of a knowledge base. Given an example and its target concept, these approaches attempt to find a small set of conditions within the knowledge base that prove the example is a member of that concept. These approaches utilize additional examples to refine the common conditions for membership, yielding a better overall concept.
%Unlike inductive approaches, deductive methods attempt to utilize a knowledge base in order to construct logical deductions. A major advantage of this approach is that, given a sufficiently expressive knowledge base, it can identify complex concepts and relationships with only a few examples.

%While these deductive approaches offered an alternative to inductive learning, the lack of good, comprehensive knowledge bases can be considered a major factor in their decline.
...
%Naturally, this raises the question of how we can utilize such knowledge bases without rendering the decades of advances and research invested into inductive learning approaches. To do so, we must utilize this knowledge base by generating features based on it, thus injecting it into the existing inductive learning framework. There have been several efforts in utilizing Linked Data for unsupervised feature generation. \citeA{cheng2011automatedfull}, for example, devise a framework for constructing features from linked data. By using intelligent queries, it is possible to construct features which utilize the knowledge base in a meaningful way. Despite this, the unsupervised nature of existing approaches limits us greatly when attempting to deduce more complex relationships between entities.
%In order to identify more complex hypotheses within the data, a different approach is necessary. %Such a method should work more similarly to deductive methods, attempting to locate conditions

In this work, we present a new supervised methodology for generating complex features based on a relational knowledge base. Our algorithm constructs new learning problems from existing feature values, using knowledge bases, such as the Semantic Web, as the features for this newly constructed problem.
Using common induction algorithms, we can then construct a classifier that serves as a feature for the original problem. An important aspect of this approach is that it can be applied recursively within the new learning problem, allowing for an automated, concept-driven exploration of the large space of possible complex features, similarly to deductive approaches. This allows us to discover powerful connections that an unsupervised approach would have difficulty discovering.
...
However, if we assume that we have access to a relational knowledge base connecting surnames to common countries of origin, we can begin to apply knowledge-based feature generation techniques to the problem, as we can move from the domain of surnames to that of countries. An unsupervised feature generation algorithm might generate a feature for each country of origin, asking whether that surname is common to that country. This would generate dozens of features, most of which are irrelevant to a given example.

Using our approach, we can improve upon this. In the original problem, the node of value female contains objects of type person. Thus, our goal is to separate this set of people to those at high risk and those at low risk. Therefore, Our method defines a new learning problem with the following training set: The objects are surnames; surnames of people with the disease are labelled as positive. The features for these objects are extracted from the knowledge base- their countries of origin.
...
Suppose that we also have access to a knowledge base containing facts regarding countries, such as average temperature, precipitation, and more (one such knowledge base is DBpedia). Using such a knowledge base, an unsupervised approach might generate features looking at the properties of the countries that a given surname is common in, giving us features along the lines of ``is this surname common in a country whose population is over 80 million?". This approach would result in a potential explosion of features, most of which are irrelevant to the learning problem at hand. This problem is greatly exacerbated when the knowledge base contains many relevant relationships, potentially causing an exponential explosion of new features.

Following our approach, we can recursively apply our method while trying to learn the new problem. We create a new training set, the objects of which are countries of origin, with countries of surnames belonging to people with high risk are labelled as positive. The knowledge base regarding countries is then used to construct features for this new this training set, which can then be given as input to an induction algorithm. As a result of this process, we end up with a classifier that tries to separate between countries of origin of people at risks and those not at risk. This classifier is then used as a feature by the first level recursive algorithm. The whole process is depicted in figure \ref{fig:moving_to_lvl2}. The resulting two-level recursive classifier is depicted in figure \ref{fig:lvl2_tree}. This constructed feature allows us to concisely and accurately capture the target concept, as the construction process is guided by the target concept, in a manner similar to that of deductive approaches.
...
While this is a simple example, it showcases the fact that additional information and complex features can result in an overall simpler, more general result. It is important to mention that an unsupervised feature generation approach could also solve this problem, but would be forced to generate more features, many of which irrelevant to the problem at hand. In cases where the knowledge base is extensive and contains many connections between entities, using an unsupervised approach to try and identify complex relationships that are buried deep beneath the surface problem would be computationally expensive to locate, and difficult to evaluate.
...
Given $S_F$, we wish to find a classifier $h_{S_F}:O\rightarrow Y$ such that the generalization error $Sum(o\in O,h_{S_F}(o)\neq C(o))$ is low.
...
Intuitively, unless the domains of the knowledge base overlap with those of the feature functions, there is little we can do with this knowledge base. 
...
A good supervised feature generation algorithm based on ${\cal R}$ is one that produces lower generalization error during induction (as in regular feature generation). More formally, if given $S_F$, the induction algorithm returns the classifier $h_F:O\rightarrow Y$, and given $S_{F_{{\cal R}}}$ the induction algorithm returns $h_{F_{{\cal R}}}:O\rightarrow Y$, a good supervised feature generation algorithm based on ${\cal R}$ must satisfy $Sum(o\in O,h_F(o)\neq C(o)) \geq Sum(o\in O,h_{F_{{\cal R}}}(o)\neq C(o))$.
...
0 & \mbox{otherwise } \end{cases}$ $,f_{i,j,c}:O\rightarrow \{0,1\}$. In the example above, we might create a feature asking whether a person's continent of origin is Europe.

While this is sufficient in cases where all relations are full functions, in reality knowledge bases are rarely complete, resulting in relations which are partial functions. We shall use $\perp$ to notate \emph{undefined}. Thus, a given relation $R_j$ is a partial function $R_j:D_j\rightarrow D_{j'}\cup\{\perp\}$. For example, $R_j$ might map a disease to its preferred treatment method. Sadly, not all diseases are treatable, and we are left with a partial function which gives values only to treatable diseases.
In this case, $f_{i,j}:O\rightarrow D_{j'}\cup\{\perp\}$ is also a partial function.
During classification, we can then treat $\perp$ as an additional value, allowing us to make use of additional knowledge when it is available, without causing harm when it is not. If we wish to have binary features, we must decide whether to treat this value as the condition being unfulfilled, yielding a value of zero, or to exclude it, leading to partial features. We can also treat it as a third possible value, leading to a function $f_{i,j,c}:O\rightarrow \{0,1,\perp\}$.
...
In cases such as this, $f_{i,j}$ may be a set of values, requiring an aggregation function to be used should we wish to utilize it. There are many possible aggregation functions that we can apply on such a value set, but we shall discuss three major ones:
\begin{itemize}
...
\end{itemize}
All of the above require a predicate function $p:D_{j'}\rightarrow \{0,1\}$. In the case of $f_{i,j,c}$ defined above, that predicate would be $p(x)=c$, and thus using the ``Exists" aggregation function would result in a similar feature, as we would get $f_{i,j,c}(x)=\begin{cases} 1 &\mbox{if } c\in R_j(f_i(x))\\ 
0 & \mbox{otherwise } \end{cases}$.

Now that we have considered all possible behaviours of a given relation $R_j$ \footnote{Of course, it is possible for a non-function relation to have missing entries. In that case we can combine the solutions for both cases}, we can proceed to define the \emph{Shallow} algorithm. Given a feature $f_i$, \emph{Shallow} will go over the relations within the knowledge base and attempt to create binary features $f_{i,j,c}:O\rightarrow \{0,1\}$ through the above solutions. \emph{Shallow} then gathers all generated features and outputs them. The pseudo-code is given below.
...
approaches to traditional FG
	combinations-lfc, ficus, LDMT, deep learning
	additonal knowledge - our family

approaches to using additional info
	supervised - our family
	unsupervised: propositionalization. unsupervised stuff like ICL and SGLR and FEGELOD

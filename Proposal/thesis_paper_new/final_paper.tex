...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		\Function{DecisionTreeInduction}{$S$, $F$, ${\cal R}$, $d$}
		\If {all examples in $S$ are of same class}
		\State 
		...
		\For{$v\in Domain(f)$}
			...
			\State subTree= \Call{DecisionTreeInduction}{$S(f)$, $F\cup$localGeneratedFeatures, ${\cal R}$, $d$}
...
%TODO:these parts
%problems in alg+how we can solve them (overfitting, label requirement?)

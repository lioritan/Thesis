...
%should this be in intro? also how to better write this? make it more subtle?
...
%cut: the deep learning for FG (this should really be a related work thing), ILP relation learning methods (background!)
		...
		\begin{algorithmic}
			\Function{RI-Tree}{S, F, R, d, min-leaf} %pick feature, split until min-leaf
				\If {$|S|\leq min-leaf$}
					...
					\Return Leaf
				\EndIf
				\State newFeatures, chosenFeature= \Call{RI-ChooseFeature}{S, F, R, d, min-leaf}
				\State \Call{output}{newFeatures}
				\State
				\Return \Call{splitByFeature}{chosenFeature, S}
			\EndFunction
			...
			\EndIf
			\State addedFeatures= $\{\}$
			\While {resources not exhausted and new relevant features exist}
				\State
				...
				\State newClassifier= \Call {RI-Tree}{(newObjects,newLabels), newF, R, $d-1$, min-leaf}
				\State newFeature= function(x): return newClassifier(featureChosen(x))
				\If {\Call{Info-Gain}{newFeature, S} $>$ \Call{Best-Info-Gain}{F, S}}
					\State
					\State addedFeatures= addedFatures$\cup$ newFeature
				\EndIf
			\EndWhile
			\State
			\Return addedFeatures, \Call{Best-Info-Gain}{F$\cup$addedFeatures, S}
			\EndFunction

...
class TreeRecursiveSRLStep(object):
    ...
    def pick_split_query(self):
        ...
        for relation in self.relations.keys():    
            feature_vals=[is_in_relation(obj, self.relations[relation], relation) for obj in self.objects]
        ...
        relevant_features, relation_avg_igs =self.filter_bad_rels(relevant_features,array(relation_avg_igs))
        ...
        for relation_used_for_recursive,rel_n in worthy_relations:  
            ...
            feature_vals=[is_in_relation(obj, self.relations[relation_used_for_recursive],relation_used_for_recursive) for obj in self.objects]
...
#            observed= array([len(find(clf_labels==1)), len(find(clf_labels!=1))])
#            expected= array([len(find(self.tagging==1)), len(find(self.tagging!=1))])
...
#                    print self.transforms, relation_used_for_recursive
#                    continue
        ...
        for i,relation in enumerate(relations):
            ...
            for other_rel in self.relations.keys():
                if other_rel==relation or other_rel=='reverse_'+relation or other_rel==relation.replace('reverse_',''):
                    continue
                feature_vals=[is_in_relation(obj, self.relations[other_rel],other_rel) for obj in new_objs]
                val_lens=[len(val) for val in feature_vals]
                if sum(val_lens)>0 :
                    barf=True
                    break
...
#            print self.transforms, relation_used
#            self.chosen_query=None
#            self.justify='not good enough'
#            return None,None,None   

# -*- coding: utf-8 -*-
"""
Created on Thu Feb 05 13:17:18 2015
...
def split_and_subtree(query_chosen, recursive_step_obj):
    ...
    return query_chosen,recursive_step_obj.sons, recursive_step_obj.good_recs, recursive_step_obj.good_recs_justify
...
class TreeRecursiveSRLStep(object):
    ...
    def pick_split_query(self):
        ...
        for relation in self.relations.keys():    
            ...
            relevant_features.append(( relation, len(frozenset(flatten(feature_vals))) ))
        ...
        choices=random.choice([a for a,b in relevant_features], self.n, True)
...
class TreeRecursiveSRLClassifier(object):
    ...
    def train(self, stopthresh):
        num_nodes= 0
        depth= 0
        node= TreeRecursiveSRLStep(self.objects, self.tagging, self.relations, self.transforms, self.n, self.MAX_DEPTH, self.SPLIT_THRESH, self.logfile, stopthresh, self.cond)
        node.depth= 1
        ...
        _,sons, rec_feature, justify =node.pick_split_query()
        if len(sons.keys())==0:
    ...
    def train_vld_local(self):
        ...
        for node in self.tree_sets:
            if (len(node.objects)<self.SPLIT_THRESH or all(node.tagging==node.chosen_tag)):#consistent/too small to split 
                ...
                continue #leaf            
            _,sons,_,_ =node.pick_split_vld_local()
            if len(sons.keys())==0:
                node.justify='leafed(weird stuff)'
                node.chosen_query=None
                self.logfile.write(' '*len(self.transforms)+'node became leaf\n')
